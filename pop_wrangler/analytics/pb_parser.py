...
class PaWrangler:
    ...
    def _filter_date(self, i):
        if pd.isna(self.pdate[i]):
            return False
        elif self.min_date is None:
            self.min_date = self.pdate[i]
        elif self.min_date > self.pdate[i]:
            self.min_date = self.pdate[i]
        delta = (self.pdate[i] - self.min_date)
        if(delta.days > 45):
            return False
        else:
            return self.pdate[i]
    ...
    def _dict_mod(self, spec_dict, i):
        ...
        pick_date = self._filter_date(i)
        if pick_date:
            try:
                spec_dict[self.quant_state[0]][self.quant_state].append((new_bal, pick_date))
            except:
    ...
    def _format_df(self, df_dict, df_key, df_key_list):
        ...
        try:
            ...
            fin_df.columns = ['deficit', 'pick date']
            return fin_df
        except:
            pass
